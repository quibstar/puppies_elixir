defmodule Puppies.AdminsTest do
  use Puppies.DataCase

  alias Puppies.Admins

  import Puppies.AdminsFixtures
  alias Puppies.Admins.{Admin, AdminToken}
  ...
  describe "apply_admin_email/3" do
    ...
    test "validates email uniqueness", %{admin: admin} do
      ...
      {:error, changeset} =
        Admins.apply_admin_email(admin, valid_admin_password(), %{email: email})
    ...
    test "sends token through notification", %{admin: admin} do
      token =
        extract_admin_token(fn url ->
          Admins.deliver_update_email_instructions(admin, "current@example.com", url)
        end)
  ...
  describe "update_admin_email/2" do
    setup do
      ...
      token =
        extract_admin_token(fn url ->
          Admins.deliver_update_email_instructions(%{admin | email: email}, admin.email, url)
        end)
  ...
  describe "delete_session_token/1" do
    test "deletes the token" do
      admin = admin_fixture()
      token = Admins.generate_admin_session_token(admin)
      assert Admins.delete_session_token(token) == :ok
      refute Admins.get_admin_by_session_token(token)
    end
  end
