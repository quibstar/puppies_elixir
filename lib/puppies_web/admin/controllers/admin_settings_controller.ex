defmodule PuppiesWeb.AdminSettingsController do
  ...
  def update(conn, %{"action" => "update_email"} = params) do
    ...
    case Admins.apply_admin_email(admin, password, admin_params) do
      {:ok, applied_admin} ->
        Admins.deliver_update_email_instructions(
          applied_admin,
          admin.email,
  ...
  def confirm_email(conn, %{"token" => token}) do
    case Admins.update_admin_email(conn.assigns.current_admin, token) do
      :ok ->
        conn
