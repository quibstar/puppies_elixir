defmodule PuppiesWeb.ListingsOnHold do
  @moduledoc """
  Loading component
  """
  ...
  def render(assigns) do
    ~H"""
      <section aria-labelledby="applicant-information-title" x-data="{ show_drawer: false, show_modal: false }">
        ...
        <%= if @listings == [] do %>
          <p class="mt-1 max-w-2xl text-gray-500">You currently don't have any on hold/sale pending listings.</p>
        <% else %>
            <table class="min-w-full divide-y divide-gray-300">
              ...
              <%= for listing <- @listings do %>
                <tr>
                  ...
                  <td class="whitespace-nowrap py-4 px-3 text-sm text-green-500 capitalize">
                    <%= live_patch listing.status, to: Routes.live_path(@socket, PuppiesWeb.ListingsStatusUpdateForm, listing.id), class: "underline ml-2 text-primary-600 hover:text-primary-900" %>
                  </td>
