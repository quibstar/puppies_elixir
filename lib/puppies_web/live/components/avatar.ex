      ...
      <div class="relative">
        <%= cond do %>
          <% !is_nil(@business) && !is_nil(@business.photo)  && Map.has_key?(@business.photo, :url) -> %>
            <%= img_tag( @business.photo.url, class: "#{@extra_classes} mx-auto h-#{@square} w-#{@square} rounded-full border border-2 border-primary-500 object-cover") %>
          ...
          <% Map.has_key?(@user.photo, :url) -> %>
            <%= img_tag( @user.photo.url, class: "#{@extra_classes} mx-auto h-#{@square} w-#{@square} rounded-full border border-2 border-primary-500 object-cover") %>
