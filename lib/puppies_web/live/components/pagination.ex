    ...
    <div id='pagination' class="mx-auto my-4">
      <div id='pagination-links' class="flex justify-center">
        <%= if @pagination.previous > 0 do %>
          ...
          <%= live_patch to: url_builder(@socket, @end_point, @segment_id, "1" , @params), class: "hover:underline mx-2 cursor-pointer" do %>
          ...
          <%= live_patch to: url_builder(@socket, @end_point, @segment_id, @pagination.previous, @params), class: "hover:underline mx-2 cursor-pointer" do %>
            <svg class="h-5 w-5 mt-0.5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <%= for i <- @pagination.first_link..@pagination.last_link do %>
          ...
          <% else %>
            <%= live_patch to: url_builder(@socket, @end_point, @segment_id, i, @params), class: "hover:underline mx-2 cursor-pointer" do %>
              <%= i %>
            <% end %>
          <% end %>
        <% end %>
        ...
        <%= if @pagination.next <= @pagination.last_page do %>
          <%= live_patch to: url_builder(@socket, @end_point, @segment_id, @pagination.next, @params), class: "hover:underline mx-2 cursor-pointer" do %>
            <svg class="h-5 w-5 mt-0.5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
            </svg>
          <% end %>

          <%= live_patch to: url_builder(@socket, @end_point, @segment_id, @pagination.last_page, @params), class: "hover:underline mx-2 cursor-pointer" do %>
            <svg class="h-5 w-5 mt-0.5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
