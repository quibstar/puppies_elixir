defmodule Puppies.Admins.AdminToken do
  use Ecto.Schema
  import Ecto.Query
  ...
  def build_session_token(admin) do
    ...
    {token, %Puppies.Admins.AdminToken{token: token, context: "session", admin_id: admin.id}}
  end
  ...
  def token_and_context_query(token, context) do
    from Puppies.Admins.AdminToken, where: [token: ^token, context: ^context]
  end
  ...
  def admin_and_contexts_query(admin, :all) do
    from t in Puppies.Admins.AdminToken, where: t.admin_id == ^admin.id
  end

  def admin_and_contexts_query(admin, [_ | _] = contexts) do
    from t in Puppies.Admins.AdminToken, where: t.admin_id == ^admin.id and t.context in ^contexts
  end
end
